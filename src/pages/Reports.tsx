...
export default function Reports() {
  ...
  useEffect(() => {
    if (employees.length > 0) {
      ...
      const months = [];
      let cumulativeCount = employees.filter(
        (e) => e.admission_date && new Date(e.admission_date) < subMonths(new Date(), 6)
      ).length;
  ...
  const activeEmployees = employees.filter((emp) =>
    ["active", "ativo"].includes(normalizeStatus(emp.status))
  ).length;
  const awayEmployees = employees.filter((emp) =>
    ["vacation", "leave", "férias", "afastado"].includes(normalizeStatus(emp.status))
  ).length;
  ...
  const latestReviewDate = reviews.length
    ? format(new Date(reviews[0].created_at), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })
    : null;
  ...
  const financialStats = employees.reduce(
    (acc, emp) => {
      ...
      try {
        ...
        /* JSON inválido – mantém valor zerado */
      }
  ...
  const generatePDF = async () => {
    ...
    const empData = [
      ...
      [
        ...
        selectedEmployee.admissionDate
          ? format(new Date(selectedEmployee.admissionDate), "dd/MM/yyyy")
          : "-",
        "Tipo Contrato:",
    ...
    const financialData = [
      [
        "Salário Base:",
        `R$ ${Number(selectedEmployee.baseSalary).toFixed(2)}`,
        "Chave PIX:",
        selectedEmployee.pixKey || "-",
      ],
      [
        "Insalubridade:",
        selectedEmployee.hasInsalubrity
          ? `Sim (R$ ${Number(selectedEmployee.insalubrity_amount || 0).toFixed(2)})`
          : "Não",
        "Adicional Noturno:",
        selectedEmployee.hasNightShift
          ? `Sim (R$ ${Number(selectedEmployee.night_shift_amount || 0).toFixed(2)})`
          : "Não",
      ],
      [
        "Salário Família:",
        `R$ ${Number(selectedEmployee.family_salary_amount || 0).toFixed(2)}`,
        "Horas Extras:",
        `R$ ${Number(selectedEmployee.overtime_amount || 0).toFixed(2)}`,
      ],
      [
        "Férias (Valor):",
        `R$ ${Number(selectedEmployee.vacation_amount || 0).toFixed(2)}`,
        "1/3 Férias:",
        `R$ ${Number(selectedEmployee.vacation_third_amount || 0).toFixed(2)}`,
      ],
      [
        "Descontos Fixos:",
        `R$ ${Number(selectedEmployee.fixed_discounts || 0).toFixed(2)}`,
        "",
        "",
      ],
    ];
    ...
    if (reviewData.length > 0) {
      ...
      if (finalY + 50 < 280) {
        ...
        sortedReviews.forEach((r, i) => {
          ...
          doc.text(r.period.split(" ")[0], x + barWidth / 2, chartY + chartHeight + 4, {
            align: "center",
          });
        });
      }
    ...
    doc.text(
      (companySettings?.company_name || "EMPRESA").substring(0, 25),
      stampX + stampWidth / 2,
      stampY + 6,
      { align: "center" }
    );
    ...
    doc.text(
      companySettings?.cnpj || "CNPJ: 00.000.000/0001-00",
      stampX + stampWidth / 2,
      stampY + 10,
      { align: "center" }
    );
    ...
    const empStampWidth = 40;
    const empStampHeight = 22;
    const empStampX = 14;
    const empStampY = finalY - 10; // Alinhado na base com o carimbo do empregador
    ...
    doc.text(
      selectedEmployee.name.length > 20
        ? selectedEmployee.name.substring(0, 20) + "..."
        : selectedEmployee.name,
      empStampX + empStampWidth / 2,
      empStampY + 12,
      { align: "center" }
    );

    doc.setFontSize(5);
    doc.text("ASSINADO DIGITALMENTE", empStampX + empStampWidth / 2, empStampY + 18, {
      align: "center",
    });
      ...
      <div className="space-y-6">
                ...
                <CardContent className="flex items-center gap-3 p-4">
                  ...
                  <div>
                    <p className="text-lg font-bold text-foreground">
                      R$ {(financialStats.payroll / 1000).toFixed(1)}k
                    </p>
                ...
                <CardContent className="flex items-center gap-3 p-4">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-foreground">
                      {financialStats.vacationDueCount}
                    </p>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Selecione um colaborador para visualizar e exportar seu dossiê completo.
            </CardDescription>
          </CardHeader>
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {selectedEmployee.admission_date
                            ? format(new Date(selectedEmployee.admission_date), "dd/MM/yyyy")
                            : "-"}
                        </p>
                      </div>
                    </CardContent>
                              ...
                              {latestEmployeeRequests.map((request) => (
                                <TableRow key={request.id}>
                                  ...
                                  <TableCell className="text-xs text-muted-foreground">
                                    {format(new Date(request.start_date), "dd/MM/yy")} -{" "}
                                    {format(new Date(request.end_date), "dd/MM/yy")}
                                  </TableCell>
                                  <TableCell>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-4">
            <div className="space-y-2">
              <h3 className="font-medium text-center text-sm text-muted-foreground flex items-center justify-center gap-2">
                <PieChartIcon className="h-4 w-4" /> Distribuição por Departamento
              </h3>
                ...
                <div style={{ width: "100%", height: 300 }}>
                  <ResponsiveContainer>
                    <PieChart>
                      ...
                      <Tooltip
                        formatter={(value) =>
                          `${value} (${(((value as number) / totalEmployeesForPercent) * 100).toFixed(1)}%)`
                        }
                      />
                      <Legend />
                    </PieChart>
            ...
            <div className="space-y-2">
              <h3 className="font-medium text-center text-sm text-muted-foreground flex items-center justify-center gap-2">
                <BarChart2 className="h-4 w-4" /> Crescimento do Headcount (Últimos 6 Meses)
              </h3>
