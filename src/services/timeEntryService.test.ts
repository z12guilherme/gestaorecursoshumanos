...
describe("timeEntryService", () => {
  describe("calculateDailyHours", () => {
    ...
    it("deve calcular horas corretamente para um turno simples", () => {
      ...
      // 9h de trabalho
      expect(timeEntryService.calculateDailyHours(entries)).toBe("9h 00m");
    });
    ...
    it("deve descontar o intervalo de almoço", () => {
      ...
      // Total 8h
      expect(timeEntryService.calculateDailyHours(entries)).toBe("8h 00m");
    });

    it("deve ignorar entradas incompletas (sem par de fechamento)", () => {
      const entries: TimeEntry[] = [{ id: "1", type: "in", timestamp: "2024-01-01T09:00:00Z" }];
