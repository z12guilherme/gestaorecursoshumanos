...
import { Briefcase, MapPin, Clock, ArrowLeft, UploadCloud, FileText, CheckCircle2, Building, X } from "lucide-react";
// Assumindo que você tenha um hook de toast configurado:
...
export default function JobDetails() {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            if (file.type === "application/pdf") {
                ...
                alert("Por favor, envie apenas arquivos PDF."); // Use seu componente de Toast aqui
            }
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert("Candidatura enviada com sucesso! Boa sorte."); // Use seu componente de Toast aqui
            setIsDialogOpen(false);
            setFormData({ name: "", email: "", phone: "" });
            setResumeFile(null);
        } catch (error) {
            console.error("Erro ao se candidatar:", error);
            alert("Ocorreu um erro ao enviar sua candidatura."); // Use seu componente de Toast aqui
        } finally {
    ...
    if (loading) {
        return <div className="flex min-h-screen items-center justify-center text-muted-foreground">Carregando detalhes...</div>;
    }
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                        <div>
                            ...
                            <div className="flex flex-wrap gap-4 text-slate-600 dark:text-slate-400 mt-4">
                                <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-full text-sm font-medium">
                                    <Building className="h-4 w-4 text-primary" /> {job.department}
                                </div>
                                <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-full text-sm font-medium">
                        ...
                        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                            <DialogTrigger asChild>
                                <Button size="lg" className="w-full md:w-auto h-12 px-8 text-base shadow-lg hover:shadow-xl transition-all" disabled={job.status !== "Aberta"}>
                            ...
                            <CardContent className="p-6 md:p-8 space-y-8">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold mb-4 flex items-center gap-2 border-b pb-2">
                                        <Briefcase className="h-5 w-5 text-primary" /> Descrição da Posição
                                    </h2>
                                ...
                                <div className="space-y-5 text-sm">
                                    ...
                                    <div className="pt-4">
                                        <Button
                                            ...
                                            variant={job.status === "Aberta" ? "default" : "secondary"}
                                            disabled={job.status !== "Aberta"}
                                        >
                                            {job.status === "Aberta" ? "Me candidatar" : "Vaga Indisponível"}
                                        </Button>
                                    </div>
                                </div>
                            </CardContent>
